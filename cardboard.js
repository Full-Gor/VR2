...
AFRAME.registerComponent('cardboard-stereo', {
  ...
  enterStereo: function (btn, divider) {
    ...
    // Si AR est actif, passer en mode stereo AR (2 feeds camera cote a cote)
    if (this._arWasActive && window.arMode) {
      window.arMode.enterStereo();
      this.renderer.setClearColor(0x000000, 0);
      this.renderer.setClearAlpha(0);
      // Garder sky/sol caches
  ...
  exitStereo: function (btn, divider) {
    ...
    // Si AR etait actif avant le VR, revenir au mode AR plein ecran
    if (this._arWasActive && window.arMode && window.arMode.isActive()) {
      window.arMode.exitStereo();
      this.renderer.setClearColor(0x000000, 0);
      this.renderer.setClearAlpha(0);
      // Garder sky/sol caches
      var sky = document.querySelector('a-sky');
